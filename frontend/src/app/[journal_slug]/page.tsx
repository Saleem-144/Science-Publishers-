...
type TabType = 'home' | 'aims' | 'editorial' | 'about' | 'indexing' | 'thematic';
...
export default function JournalPage() {
  ...
  const groupedArticles = articlesList.reduce((acc: any, article: any) => {
    const volNum = article.volume?.volume_number || 'Unknown Volume';
    const issueNum = article.issue?.issue_number || 'Unknown Issue';
    const year = article.volume?.year || '';
                  ...
                  <div key={designation} className="space-y-6">
                    ...
                    <div className="divide-y divide-gray-100">
                      {members.map((member: any) => (
                        <div key={member.id} className="flex flex-col md:flex-row gap-4 py-3 first:pt-0 last:pb-0 transition-all hover:bg-gray-50/50 px-2 -mx-2 rounded-lg items-start">
                          {member.image_url && (
                            <div className="flex-shrink-0 w-full md:w-32 lg:w-36">
                          ...
                          <div className="flex-1 min-w-0 flex flex-col pt-1">
                            <h4 className="text-lg font-bold text-gray-900 leading-tight mb-0.5">{member.name}</h4>
                            <div className="text-gray-600 space-y-0.5">
                              {member.department && (
                                <p className="text-[13px] font-medium text-academic-navy/80 leading-tight">{member.department}</p>
                              )}
                              {member.institution && (
                                <p className="text-[13px] leading-tight">{member.institution}</p>
                              )}
                              {member.country && (
                                <p className="text-[10px] font-bold uppercase tracking-widest text-academic-gold mt-1">{member.country}</p>
                              )}
                            </div>
                            {member.description && (
                              <div 
                                className="ql-editor !p-0 mt-1 text-gray-700 leading-tight text-[13px] italic border-l-2 border-academic-gold/20 pl-3 editorial-description"
      ...
      case 'indexing':
        return (
          <div className={contentClasses}>
            ...
            {journal.indexing_entries && journal.indexing_entries.length > 0 ? (
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
                {journal.indexing_entries.map((entry: any) => (
                  <div key={entry.id} className="flex flex-col items-center text-center space-y-3 group">
                    <div className="relative w-full aspect-[3/2] bg-white rounded-lg border border-gray-100 shadow-sm p-4 flex items-center justify-center transition-all group-hover:shadow-md group-hover:border-academic-blue/30">
                      {entry.logo_url ? (
                        ...
                        <span className="font-bold text-gray-400 text-sm">{entry.title}</span>
                      )}
                    </div>
                    <div>
                      <p className="text-sm font-bold text-gray-900 leading-tight">{entry.title}</p>
                      {entry.url && (
                        <a 
                          href={entry.url} 
                          target="_blank" 
                          rel="noopener noreferrer" 
                          className="text-[11px] text-academic-blue font-bold uppercase tracking-wider hover:underline"
                        >
                          Verify Listing
                        </a>
                      )}
          ...
          <div className="flex flex-col md:flex-row gap-8 items-center">
            {journal.cover_image && (
              <div className="flex-shrink-0 w-40 h-52 relative rounded-lg overflow-hidden shadow-2xl border-2 border-white/20">
                <Image src={journal.cover_image} alt={journal.title} fill className="object-cover" />
              </div>
            )}
                  ...
                  <div className="divide-y divide-gray-50">
                    {volumesList.map((vol: any) => (
                      <div key={vol.id} className="p-4 hover:bg-gray-50 transition-colors">
                        <Link href={`/${journalSlug}/volumes`} className="block group">
